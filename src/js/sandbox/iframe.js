...
window.addEventListener("message", event => {
  ...
  if (!type || !Boolean(~ALLOWED_TYPES.indexOf(type))) {
    // eslint-disable-line
    return;
  }
...
window.onload = () =>
  window.parent && window.parent.postMessage({ LOADED: true }, "*"); // eslint-disable-line
...
$bus.on(MESSAGES.RUN_SCRIPT, code => {
  ...
  const scriptText = document.createTextNode(
    `try { ${transformedOutput} } catch (e) { console.error(e) }`
  );
