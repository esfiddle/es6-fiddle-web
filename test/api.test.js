  ...
  it("Should Update existing fiddle for user1", done => {
    ...
    agent
      ...
      .end((err, res) => {
        ...
        return Fiddles.findOne(
          { fiddle: newFiddle.fiddle },
          (findErr, item) => {
            if (findErr) {
              return done(findErr);
            }
...
describe("GET /fiddles/fiddle", () => {
  it("should get correct fiddle from DB", done => {
    request(app)
      .get(`/fiddles/${testFiddle.fiddleGuest.fiddle}`)
      .expect(200)
      .expect(res => {
  ...
  it("should not return fiddle", done => {
    request(app)
      .get(`/fiddles/${parseInt(Date.now(), 10).toString(36)}`)
      .expect(404)
      .expect(res => {
...
describe("POST /star/:fiddle", () => {
  ...
  it("Should Star fiddle", done => {
    agent
      .post(`/star/${testFiddle.fiddleU1.fiddle}`)
      .expect(200)
      .end(err => {
        if (err) {
          return done(err);
  ...
  it("Should return 400 unknown fiddle", done => {
    agent
      .post(`/star/${parseInt(Date.now(), 10).toString(36)}`)
      .expect(400)
      .end(err => {
        if (err) {
          return done(err);
...
describe("POST /gist/:fiddle unauthorized", () => {
  ...
  it("should return 401 for unauthorized user", done => {
    request(app)
      .post(`/gist/${testFiddle.fiddleU1.fiddle}`)
      .expect(401)
      .end(err => {
        if (err) {
          return done(err);
  ...
  it("Should return 400 unknown fiddle", done => {
    agent
      .post(`/gist/${parseInt(Date.now(), 10).toString(36)}`)
      .expect(400)
      .end(err => {
        if (err) {
          return done(err);
...
describe("POST /private/:fiddleID", () => {
  ...
  it("Should make fiddle private", done => {
    agent
      .post(`/private/${testFiddle.fiddleU1.fiddle}`)
      .expect(200)
      .expect(res => {
  ...
  it("should return 400 of user trying to make another user's fiddle private", done => {
    agent
      .post(`/private/${testFiddle.fiddleU2.fiddle}`)
      .expect(400)
      .end(err => {
        if (err) {
          return done(err);
  ...
  it("should return 400 of user trying to make another anonymous fiddle private", done => {
    agent
      .post(`/private/${testFiddle.fiddleGuest.fiddle}`)
      .expect(400)
      .end(err => {
        if (err) {
          return done(err);
  ...
  describe("GET /fiddles/fiddle for private fiddle", () => {
    it("should get private fiddle for logged in user.", done => {
      agent
        .get(`/fiddles/${testFiddle.fiddleU1.fiddle}`)
        .expect(200)
        .expect(res => {
      ...
      request(app)
        ...
        .expect(res => {
          expect(res.body.message).to.equal(
            "This is a private fiddle please login."
          );
        })
        .end(err => {
          if (err) {
    ...
    describe("GET /fiddles/fiddle for private fiddle", () => {
      ...
      it("should return 400 if logged in user trying to access other user's private fiddle", done => {
        ...
        Fiddles.findOneAndUpdate(
          ...
          agent
            .get(`/fiddles/${testFiddle.fiddleU2.fiddle}`)
            .expect(400)
            .end(err => {
              if (err) {
                return done(err);
